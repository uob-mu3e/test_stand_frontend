...
static void free_mem(HV_INFO * hv_info)
{
   ...
   free(hv_info->current_limit_ramping);
...
void hv_demand(INT hDB, INT hKey, void *info)
{
   ...
   for (i = 0; i < hv_info->num_channels; i++) {
      /* if device can do hardware ramping, just set value, and set ramping current limit. It's up to the device driver to do something with that */   
      if (hv_info->driver[i]->flags & DF_HW_RAMP) {
         if (hv_info->demand[i] != hv_info->demand_mirror[i]) {
			 device_driver(hv_info->driver[i], CMD_SET_CURRENT_LIMIT,i - hv_info->channel_offset[i], hv_info->current_limit_ramping[i]); 
            device_driver(hv_info->driver[i], CMD_SET, i - hv_info->channel_offset[i], hv_info->demand[i]);
            hv_info->last_change[i] = ss_millitime();
...
void hv_set_chState(INT hDB, INT hKey, void *info)
{
   ...
   for (i = 0; i < hv_info->num_channels; i++){
      ...
      device_driver(hv_info->driver[i], CMD_SET_CHSTATE, i - hv_info->channel_offset[i], (float)hv_info->chState[i] );
   }
...
INT hv_init(EQUIPMENT * pequipment)
{
   ...
   hv_info->current_limit_ramping = (float *) calloc(hv_info->num_channels, sizeof(float));
   ...
   /* Current limit for Ramping*/
   validate_odb_array(hDB, hv_info, "Settings/Ramp Current Limit", 3000, CMD_GET_CURRENT_LIMIT, 
                      hv_info->current_limit_ramping, hv_set_current_limit, pequipment);
   ...
   for (i = 0; i < hv_info->num_channels; i++) {
      if ((hv_info->driver[i]->flags & DF_PRIO_DEVICE) == 0) {
         if( fabs(hv_info->demand[i]) < fabs(hv_info->voltage_limit[i])  ) { hv_info->demand_mirror[i] = hv_info->demand[i]; }
           else { hv_info->demand_mirror[i] = hv_info->voltage_limit[i]; }
         status = device_driver(hv_info->driver[i], CMD_SET, 
                                i - hv_info->channel_offset[i], hv_info->demand_mirror[i]);
         ...
         if (hv_info->driver[i]->flags & DF_REPORT_CHSTATE)
            status = device_driver(hv_info->driver[i], CMD_SET_CHSTATE,
                                   i - hv_info->channel_offset[i], (float)hv_info->chState[i]);
      } else {
...
INT hv_idle(EQUIPMENT * pequipment)
{
   ...
   /* check current limit during ramping */
   if(DF_HW_RAMP)
   {
     for (i = 0; i < hv_info->num_channels; i++)
     { 
	   float current_limit_driver; //ask current limit from driver
	   device_driver(hv_info->driver[i], CMD_GET_CURRENT_LIMIT,i - hv_info->channel_offset[i], &current_limit_driver);
	   if(hv_info->current[i] < hv_info->current_limit[i] & current_limit_driver > 1.001*hv_info->current_limit[i]) device_driver(hv_info->driver[i], CMD_SET_CURRENT_LIMIT,i - hv_info->channel_offset[i], hv_info->current_limit[i]);
	   printf("decreasing current limit from %f to %f\n",current_limit_driver,hv_info->current_limit[i]);     
     }
   }
   return status;
}
