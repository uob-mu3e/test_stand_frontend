...
outfile = open("update_DACs_from_QC_fixed.msl", 'w')
outfile.write("COMMENT Script for updating VPVCO and VNVCO and link masks from the result of QC test. Autogenerated from update_DACS_from_linkquality.py\n")
...
for chip_number in range(120):
    ...
    if opt_index == -1:
        print("Chip Number", g_chip_number, " could not be optimized!")
        ...
        #client.odb_set("/Equipment/PixelsCentral/Settings/BIASDACS/" + str(g_chip_number) + "/BiasBlock_on", 5)
        #outfile.write("ODBSET /Equipment/PixelsCentral/Settings/BIASDACS/" + str(g_chip_number) + "/BiasBlock_on, 5\n")
        continue
    else:
        if af0[opt_index] > 10:
            not_working_links[-1].append((chip_number%12)*3 + 0)
    ...
    client.odb_set("/Equipment/PixelsCentral/Settings/BIASDACS/" + str(g_chip_number) + "/VPVCO", opt_value)
    client.odb_set("/Equipment/PixelsCentral/Settings/BIASDACS/" + str(g_chip_number) + "/VNVCO", opt_value+1)
    outfile.write("ODBSET /Equipment/PixelsCentral/Settings/BIASDACS/" + str(g_chip_number) + "/VPVCO, " + str(opt_value) + "\n")
    outfile.write("ODBSET /Equipment/PixelsCentral/Settings/BIASDACS/" + str(g_chip_number) + "/VNVCO, " + str(opt_value+1) + "\n")
    max_value = max(qf0)
    plt.plot(opt_value, max_value, marker="o", markersize=7, color = color)
    plt_legend.append("VPVCO Chip " + str(g_chip_number))

print("Number of working links:", n_working_links)
...
for feb, nwl in enumerate(not_working_links):
    ...
    outfile.write("ODBSET /Equipment/PixelsCentral/Settings/FEBS/" + str(feb) + "/MP_LVDS_LINK_MASK, " + str(mask) + "\n")
    ...
    outfile.write("ODBSET /Equipment/PixelsCentral/Settings/FEBS/" + str(feb) + "/MP_LVDS_LINK_MASK2, " + str(mask2) + "\n")
